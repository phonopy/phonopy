...
def move_force_dataset_to_mlp_dataset(phonon: Phonopy) -> None:
    """Move a force dataset to mlp_dataset for MLP training.

    A dataset with forces is the MLP training dataset, so it is moved to
    mlp_dataset. A displacement-only dataset (e.g. from phonopy_disp.yaml)
    has no forces and is discarded: an existing MLP (polymlp.yaml) is loaded
    instead, and new displacements/forces are generated by it. In both cases
    phonon.dataset is cleared so that it is not mistaken for an evaluated
    dataset later on.
