...
- default: force sets come from the pre-computed displaced supercells in
...
- --from-mlp POLYMLP: force sets are evaluated on the fly from a
  ...
  the static single points in static-grid/grid-NNN.
...
Only the VASP calculator is supported: the static internal energy U(a, c),
the forces, and the electronic states are read from VASP outputs
(vaspout.h5 / vasprun.xml). Other calculators are rejected because phonopy has
no interface yet to read their static single-point energy.
...
def read_electronic_states(sdir: str) -> ElectronicStates | None:
    """Read electronic states from static-grid/grid-NNN/vaspout.h5 if available.

    Returns the ElectronicStates when vaspout.h5 exists and carries the electron
    eigenvalues, otherwise None. A missing vaspout.h5 (only vasprun.xml was
    written) or one without eigenvalues is not an error: the grid point is built
    without F_el and a notice is printed. This lets the electronic term default
    to on while degrading gracefully where it is unavailable.

    """
    path = os.path.join(sdir, "vaspout.h5")
    if not os.path.exists(path):
        print(f"  {sdir}: no vaspout.h5; electronic states (F_el) not stored")
        return None
    try:
        return electronic_states_from_vaspout(path)
    except (KeyError, OSError):
        print(f"  {sdir}: vaspout.h5 has no electron eigenvalues; F_el not stored")
        return None
...
def build_dft_grid_point(
    ...
    electronic = read_electronic_states(sdir) if with_electronic else None
...
def get_options() -> Namespace:
    ...
    parser.add_argument(
        "--phonon-grid",
        default="phonon-grid",
        help="directory of pre-computed displaced supercells, used when "
        "--from-mlp is not given (default: phonon-grid)",
    )
    parser.add_argument(
        "--distance",
        type=float,
    ...
    parser.add_argument(
        ...
        dest="electronic",
        action="store_false",
        help="do not store electronic states (for F_el) even when static-grid "
...
def run() -> None:
    ...
    if calculator != "vasp":
        raise SystemExit(
            f"phonopy-anisotropic-qha-dataset supports only the VASP calculator, but "
            f"the reference '{args.filename}' specifies '{calculator}'. The static "
            f"internal energy U(a, c) and the electronic states are read from VASP "
            f"outputs (vaspout.h5 / vasprun.xml) only."
        )
    ...
    if args.from_mlp:
        ...
        print(f"Reading pre-computed forces from {args.phonon_grid}/")
