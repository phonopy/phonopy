...
An experimental Rust backend (`phonors`) is also available, including a
build mode that skips the C extension entirely
(`PHONOPY_NO_C_EXT=1`). See {ref}`rust_backend` for installation and
usage.
