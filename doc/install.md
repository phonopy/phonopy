...
## Computational backend (C and experimental Rust)

Phonopy ships two implementations of its compute-heavy kernels (dynamical
matrix builders, dipole-dipole, force-constant symmetrization, BZ-grid
helpers, smallest-vector search, tetrahedron-method weights, ...).  The
default is the C extension that is built together with the Python package;
the Rust implementation is provided through a separate package called
[`phonors`](https://github.com/phonopy/phonors) and is **experimental**
at the moment.
...
(install_lang_dispatch)=

### Selecting the backend (`lang` keyword and `--rust`)
...
The choice propagates through the major helpers (`Phonopy`, `Primitive`,
`Symmetry`, `BZGrid`, `DynamicalMatrix`, `Mesh`, `ThermalProperties`,
`TotalDos`, `ProjectedDos`, ...), and the resolved backend is exposed
on `Phonopy.lang`.  At the command line, the `--rust` flag flips the
...
To trace which backend each lang-aware call site picks, set
`PHONOPY_TRACE_LANG=1` before running.  Messages are emitted on the
`phonopy.lang` Python logger at DEBUG level (a stderr handler is
configured automatically by the env var).
...
### Installing the experimental Rust backend (`phonors`)

`phonors` is not on PyPI or conda-forge yet; it is built from source via
[maturin](https://maturin.rs).  A working Rust toolchain (`rustc` and
`cargo`) and `maturin` are required, both available from conda-forge:
...
Python environment as an editable build; re-run it whenever the Rust
sources change.  After that, `import phonors` succeeds from anywhere
and `Phonopy(lang="Rust")` (or `--rust`) becomes available.
...
`Phonopy()` and `phonopy.load()` detect the missing C extension, emit a
one-time `[phonopy] C extension ... is not available; falling back to
lang='Rust' ...` message, and silently flip `lang="C"` requests to
`lang="Rust"` -- so `phonors` becomes a hard requirement for this
build.  An informative `ImportError` is raised if neither backend is
available.

The CLI logo also adapts: with the Rust backend you will see

```text
Rust backend (phonors) using rayon (10 threads).
```

instead of the OpenMP banner from the C build.  The thread count
follows rayon's defaults (overridable via the `RAYON_NUM_THREADS`
