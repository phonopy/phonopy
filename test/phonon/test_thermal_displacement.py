...
def test_ThermalDisplacements(ph_nacl):
    ...
    # t_min, t_max, t_step
    ph_nacl.run_thermal_displacements(t_min=0,
                                      t_max=901,
                                      t_step=100,
                                      freq_min=1e-2)
    ...
    ph_nacl.run_thermal_displacements(temperatures=temperatures,
                                      ...
                                      freq_min=1e-2)
    td = ph_nacl.thermal_displacements
    np.testing.assert_allclose(td_ref[:, [0, 3]], td.thermal_displacements,
                               atol=1e-5)


def test_ThermalDisplacementMatrices(ph_sno2):
    ph_sno2.init_mesh([5, 5, 5],
                      with_eigenvectors=True,
                      is_mesh_symmetry=False,
                      use_iter_mesh=True)
    # t_min, t_max, t_step
    temperatures = [0, 500]
    ph_sno2.run_thermal_displacement_matrices(t_min=0,
                                              t_max=501,
                                              t_step=500,
                                              freq_min=1e-2)
